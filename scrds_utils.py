...
#
# wrongs ingest on a list of abio data provided in a csv file
...
ingestAbioDataWrapper <- function(file, batch_num, rds_location = "/scratch/nbatada/abio/batch") {
  # Read input file
  abio_datasets_to_ingest <- read.csv(file)

  # Replace dots in column names if necessary
  colnames(abio_datasets_to_ingest) <- gsub("\\.", "_", colnames(abio_datasets_to_ingest))

  # Check required columns
  required_columns <- c("dataset_ID", "data_handle", "rds_in", "rds_out", "description")
  missing_columns <- setdiff(required_columns, colnames(abio_datasets_to_ingest))
  if (length(missing_columns) > 0) {
    stop("Missing required columns in input: ", paste(missing_columns, collapse = ", "))
  }

  # Set working directory
  setwd(sprintf("%s%s", rds_location, batch_num))

  for (i in seq_len(nrow(abio_datasets_to_ingest))) {
    # Extract variables from the current row
    dataset_id <- abio_datasets_to_ingest$dataset_ID[i]
    data_subset_number <- strsplit(dataset_id, "_")[[1]][2]

    data_handle <- abio_datasets_to_ingest$data_handle[i]
    rds_in <- abio_datasets_to_ingest$rds_in[i]
    rds_out <- abio_datasets_to_ingest$rds_out[i]
    description <- abio_datasets_to_ingest$description[i]

    # Skip non-oncology datasets
    if (startsWith(rds_out, "INFLAM_") || startsWith(rds_out, "UNKNOWN_")) {
      next
    }

    # Ingest data and check status
    jid <- ingestAbioData(rds_in, rds_out, DESCRIPTION = description)
    print(jid)

    status <- checkStatus(jobId = jid)
    print(status)
  }
}
