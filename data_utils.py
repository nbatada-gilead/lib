...
def prepareData(data_handle, GENES=['CCR8', 'FOXP3', 'IFNG', 'IL17A', 'CTLA4', 'PDCD1', 'CD27', 'IKZF2', 'TNFRSF18', 'CD177', 'TNFRSF8', 'CD74', 'FUT7', 'IL1R2', 'IL12RB2', 'TNFRSF4', 'CXCR3', 'STAT1', 'IL10'], GENES_additional=None):
    ...
    if missing_files:
        raise ValueError(f"File not found ('{data_handle}'): {', '.join(missing_files)}")
    ...
    required_columns = [colname_celltype, colname_tissue, colname_group]
    
    print('required_columns=',required_columns)

    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise ValueError(f"Missing a column in the 'files.processed.meta' file: {', '.join(missing_columns)}.")
    ...
    for c in df.columns:
        try:
            ...
            print(f'Skipping: column = {c}\n{df[c].head(1)}')
            print(f'YOU MAY HAVE MULTIPLE COLUMNS WITH THE SAME NAME (in this case THIS ERROR WILL BE SHOWN TWICE)')
            pass
    ...
    # if IDX is all false, then raise an error
    if not np.any(IDX):
        print(f'[WARNING] Did not find any treg cells. colname_celltype={colname_celltype}, colname_celltype_value_treg={colname_celltype_value_treg}')
    else:
    ...
    # define other columns that of interest
    # colname_donorid
    # colname_ .. etc
...
def getAnalysisReadyData(data_handle):
    d_info=getInfo(data_handle)
    if d_info.get('files.analysis_ready', None):
        df=pd.read_csv(d_info['files.analysis_ready'], sep='\t', index_col='barcode')
        return(df)
    else:
        raise ValueError(f'getInfo for {data_handle}, needs to have "files.analysis_ready" parameter defined')
